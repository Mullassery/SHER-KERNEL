...
**Status**: 40 crates. See [README.md](README.md) for the authoritative, currently-verified test count and real-vs-simulated breakdown per crate — prose test counts in this file go stale (this line previously said 767 while README said 768); run `cargo test --workspace` for the true current number.
...
Run `cargo test --workspace` for the current, authoritative count rather than trusting any number written in prose here or elsewhere — prose test counts are exactly what went stale before (this file said 767 while README said 768 until this pass).
