...
impl MemoryLeakDetector {
    ...
    pub fn record_allocation(&mut self, driver_id: ObjectId, total_bytes: u64, current_time_ms: u64) {
        let history = self
            .allocation_history
            .entry(driver_id)
    ...
    pub fn detect_leak(&mut self, driver_id: ObjectId, current_time_ms: u64) -> Option<Anomaly> {
        if let Some(history) = self.allocation_history.get(&driver_id) {
            ...
            let bytes_delta = last.total_allocated_bytes.saturating_sub(first.total_allocated_bytes);
            let mb_delta = bytes_delta / (1024 * 1024);
            let time_delta_seconds = time_delta_ms / 1000;

            if time_delta_seconds > 0 {
                let rate_mb_per_sec = mb_delta / time_delta_seconds as u64;

                if rate_mb_per_sec > self.threshold_mb_per_second {
                    self.detections += 1;

                    let confidence = (rate_mb_per_sec as f64 / self.threshold_mb_per_second as f64).min(1.0);
                    ...
                    metrics.insert("total_allocated_mb".to_string(), (last.total_allocated_bytes / (1024 * 1024)) as f64);
                    metrics.insert("growth_percentage".to_string(), (rate_mb_per_sec as f64 / self.threshold_mb_per_second as f64) * 100.0);
...
impl InterruptStormDetector {
    ...
    pub fn record_interrupt(&mut self, irq: u32, timestamp_ms: u64) {
        let history = self.interrupt_counts.entry(irq).or_insert_with(VecDeque::new);
    ...
    pub fn detect_storm(&mut self, irq: u32, current_time_ms: u64) -> Option<Anomaly> {
        if let Some(history) = self.interrupt_counts.get(&irq) {
            ...
            if count > self.threshold_per_second {
                ...
                metrics.insert("excess_percentage".to_string(), ((count as f64 / self.threshold_per_second as f64) - 1.0) * 100.0);
...
impl DmaAbuseDetector {
    ...
    pub fn record_dma(&mut self, driver_id: ObjectId, bytes: u64, is_write: bool, timestamp_ms: u64) {
        let ops = self.dma_operations.entry(driver_id).or_insert_with(VecDeque::new);
    ...
    pub fn detect_abuse(&mut self, driver_id: ObjectId, current_time_ms: u64) -> Option<Anomaly> {
        if let Some(ops) = self.dma_operations.get(&driver_id) {
            ...
            if should_alert {
                ...
                metrics.insert("threshold_bytes".to_string(), self.threshold_bytes_per_second as f64);
...
impl AnomalyEngine {
    ...
    pub fn record_memory_state(&mut self, driver_id: ObjectId, total_bytes: u64, current_time_ms: u64) {
        self.memory_detector.record_allocation(driver_id, total_bytes, current_time_ms);
    ...
    pub fn record_dma_operation(&mut self, driver_id: ObjectId, bytes: u64, is_write: bool, timestamp_ms: u64) {
        self.dma_detector.record_dma(driver_id, bytes, is_write, timestamp_ms);
    ...
    pub fn get_stats(&self) -> AnomalyStats {
        let critical_count = self.recent_anomalies.iter().filter(|a| a.severity == AnomalySeverity::Critical).count();
        let high_count = self.recent_anomalies.iter().filter(|a| a.severity == AnomalySeverity::High).count();
