...
impl AdaptiveScheduler {
    ...
    pub fn decide_scheduling(
        ...
        decision.strategy = match (cpu_usage, memory_usage, recent_anomalies) {
            ...
            (cpu, _, anomalies)
                if cpu > 80.0 && latency_observed_ms < target_latency_slo_ms && anomalies <= 2 =>
            {
            ...
            (_, _, _) if latency_observed_ms > target_latency_slo_ms * 2.0 => {
                decision.reason = format!(
                    "Severe latency: {}ms vs {}ms SLO",
                    latency_observed_ms, target_latency_slo_ms
                );
        ...
        decision.preempt_other =
            decision.strategy == SchedulingStrategy::RealTime && recent_anomalies > 0;
    ...
    pub fn record_slo_result(
        ...
        let metrics = self
            .metrics
            .entry(driver_id)
            .or_insert_with(|| SchedulingMetrics {
                driver_id,
                decisions_made: 0,
                strategy_switches: 0,
                slo_violations: 0,
        ...
        metrics.slo_achievement_rate = if metrics.decisions_made > 0 {
            ((metrics.decisions_made - metrics.slo_violations) as f64
                / metrics.decisions_made as f64)
                .max(0.0)
        } else {
            1.0
        };
...
impl WorkloadClassifier {
    ...
    pub fn classify(
        ...
        if let Some(existing) = self.profiles.get_mut(&driver_id) {
            ...
            existing.memory_intensity =
                existing.memory_intensity * (1.0 - alpha) + memory_intensity * alpha;
            existing.io_intensity = existing.io_intensity * (1.0 - alpha) + io_intensity * alpha;
            existing.latency_sensitivity =
                existing.latency_sensitivity * (1.0 - alpha) + latency_sensitivity * alpha;
            existing.samples += 1;
